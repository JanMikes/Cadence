...
export const AGENT_PROMPTS: Record<string, AgentPromptDef> = {
  ...
  planner: {
    ...
    defaultTemplate: [
      ...
      "or irreversible (set risky:true). If the deliverable includes non-code assets (reports, PDFs,",
      "documents, exports), plan those steps to write the files into the task outputs directory given",
      "in your context — never into the repository.",
      "DO NOT WRITE CODE — you are read-only (plan mode). Output JSON only.",
      "Respond with ONLY this JSON shape:",
  ...
  implementer: {
    ...
    defaultTemplate: [
      ...
      "Non-code deliverables (reports, PDFs, documents, exports) belong in the task outputs",
      "directory given in your context — never commit generated assets to the repository.",
  ...
  verifier: {
    ...
    defaultTemplate: [
      ...
      "criterion is met. If the task's deliverable includes non-code outputs (reports, PDFs, exports),",
      "confirm they exist in the task outputs directory given in your context and satisfy the criteria —",
      "and flag generated assets that were wrongly committed to the repo.",
      "Report pass/fail with specifics — DO NOT FIX, only report. Output JSON only.",
      "Respond with ONLY this JSON shape:",
  ...
  delivery: {
    ...
    variables: [
      ...
      { name: "outputsLine", doc: "`OUTPUT FILES (non-code deliverables, linked on the task): …` when the task produced outputs; empty otherwise (line drops)." },
    ],
    defaultTemplate: [
      "You are the Delivery agent. Produce a concise human summary of WHAT changed and WHY, referencing",
      "the acceptance criteria and the verify results. Mention any output files (non-code deliverables)",
      "by name — they are linked on the task. Be specific and skimmable. Output JSON only.",
